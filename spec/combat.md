...
**例如：** 水攻打火 → 优势 +20%，对方火抗 10% → 实际元素修正 = 1 + 0.2 - 0.1 = 1.1（+10%）
...
你可以在代码里约定：`power_multiplier` 对攻击力做线性放大；`effects` 用一个简单的解释器处理。
...
**战术价值：** 玩家要在战斗中规划技能顺序，先"铺局"积气，再一口气爆发。
...
**玩法：** 面对难命中的敏捷 BOSS 时，玩家可以选择：先观战 1～2 回合，堆洞察 → 再释放关键控制/爆发技。
